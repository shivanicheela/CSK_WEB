...
export default function Admin() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      {/* Navigation Tabs — NO Upload Videos */}
          ...
          <div className="flex gap-6 min-w-max">
            {[
              ...
              <button key={t.id} onClick={() => setActiveTab(t.id)}
                className={`py-4 px-1 font-bold text-sm border-b-4 whitespace-nowrap transition-all ${activeTab === t.id ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white'}`}>
                {t.label}
              </button>
            ))}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {activeTab === 'next-session' && (
          <div className="max-w-2xl mx-auto">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-8">
              <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-2">📅 Schedule Next Session</h2>
        ...
        {activeTab === 'mock-tests' && (
          <div className="space-y-4">
            ...
            {mockTab === 'import' && (
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 space-y-4">
                <h3 className="text-lg font-black text-gray-900 dark:text-white">Import Questions (CSV / JSON)</h3>
                <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-xl text-xs text-yellow-800">
                  <p className="font-bold mb-1">CSV Format (first row = header):</p>
                  <code className="block">question,optionA,optionB,optionC,optionD,correctIndex,explanation,category,testId</code>
                  <p className="font-bold mt-2 mb-1">OR paste a JSON array:</p>
                  <code className="block">{`[{"question":"...","options":["A","B","C","D"],"correctAnswer":1,"explanation":"...","category":"Polity","testId":"gs-paper-1"}]`}</code>
                </div>
                <textarea rows={12} className="w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-600 rounded-xl text-sm font-mono resize-none focus:border-indigo-400 outline-none dark:bg-gray-700 dark:text-white"
